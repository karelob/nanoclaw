...
_PERSONAL_TOP_FOLDERS = {
    'legal', 'létání - mé úřední', 'létání', 'korespondence', 'dane a finance',
    'nemovitosti', 'banky', 'dary', 'jiné', 'pujcky', 'pojisteni',
    'archiv - splacené', 'archiv',
}


def _collect_groups_personal() -> dict[str, ContractGroup]:
    """Seskupí osobní dokumenty dle nejhlubší smysluplné složky."""
    groups: dict[str, ContractGroup] = {}
    for path, source_key, category in iter_personal_files():
        rel = path.relative_to(PERSONAL_SOURCES[source_key])
        parts = [unicodedata.normalize('NFC', p) for p in rel.parts[:-1]]

        # Counterparty = nejhlubší složka, která není generická top-level
        counterparty = source_key
        for part in reversed(parts):
            if part.lower() not in _PERSONAL_TOP_FOLDERS and len(part) > 2:
                counterparty = part
                break
        if counterparty == source_key and parts:
            counterparty = parts[-1]  # alespoň nejhlubší složka

        key = f"personal/{source_key}/{counterparty}"
        source = f"iCloud/{source_key}/" + '/'.join(parts)
